...
def detect(opt):
    source, weights, view_img, save_txt, imgsz, save_txt_tidl, kpt_label, kpts_color = opt.source, opt.weights, opt.view_img, opt.save_txt, opt.img_size, opt.save_txt_tidl, opt.kpt_label, opt.kpts_color
            ...
            if len(det):
                ...
                for det_index, (*xyxy, conf, cls) in enumerate(reversed(det[:,:6])):
                    ...
                    if save_img or opt.save_crop or view_img:  # Add bbox to image
                        ...
                        plot_one_box(xyxy, im0, label=label, color=colors(c, True), line_thickness=opt.line_thickness, kpt_label=kpt_label, kpts=kpts, steps=3, orig_shape=im0.shape[:2], kpts_color_dict=kpts_color_dict)
                        if opt.save_crop:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--kpt-label', action='store_false', help='use keypoint labels')
